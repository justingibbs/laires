...
| Module / File        | Story file / Chapter / Scene                       |
...
### File Tools
- **story_grep**: Search across all project story files by text, regex, or semantic similarity.
- **read_scene**: Read a specific scene from any story file.
- **list_scenes**: List all scenes across all story files with metadata (file, word count, characters present).
- **read_context_file**: Read a supporting file (outline, character sheet, notes) for reference.
...
│  multi-  │ serde    │  Anthropic            │
│  format  │ JSON     │  Pydantic AI Gateway  │
│  parsers │ persist  │  Local (Ollama, etc.) │
├──────────┴──────────┴───────────────────────┤
│           Project Working Directory          │
│  Any mix of: .md, .fountain, .docx, .txt     │
│  Story files + outlines + notes + char sheets │
│  Manifest tracks classification & ordering    │
└─────────────────────────────────────────────┘
```
...
- **File indexing**: Custom parsers for .md/.fountain/.docx/.txt, ripgrep for text search
...
File classification and ordering is managed automatically via `.laires/manifest.toml`, generated by `laires scan` (see spec for details). Writers drop their files into the project folder — any mix of `.md`, `.fountain`, `.docx`, `.txt` — and Laires classifies them using an LLM during the scan phase.
...
- Multi-file project support (any mix of .md, .fountain, .docx, .txt)
- LLM-powered file discovery and classification (story vs. outline vs. notes)
- Manifest system for file ordering, roles, and version detection
- Cross-file scene mapping and narrative graph
...
### File Format and Structure (Resolved)
1. ~~Should Markdown be the canonical working format, with import/export for other formats?~~ **No. Laires works with whatever files the author has — .md, .fountain, .docx, .txt. No canonical format; files are read and edited in place.**
2. ~~How should scenes be delineated within files — one file per scene, per chapter, or frontmatter markers within a single file?~~ **Scenes are detected automatically per file via SceneMap (prose heuristics or Fountain headings). Writers organize files however they want (one per act, one per chapter, or a single file).**
3. ~~Should we define a frontmatter schema (YAML) for scene metadata (characters present, location, time)?~~ **Optional. SceneMap supports frontmatter but doesn't require it. Metadata is LLM-inferred by Analysis.**
4. ~~Should the graph be stored alongside the manuscript (`.laires/` directory) or separately?~~ **Alongside, in `.laires/`.**

### Writing Format Support (Resolved)
5. ~~How do we handle screenplays vs. novels vs. short stories?~~ **Per-file format detection. Each file in the manifest has a format field. SceneMap applies the appropriate parse mode per file.**
6. ~~Should we support Fountain (screenplay format) natively?~~ **Yes, native Fountain support implemented in Phase 2.**
7. ~~Do different formats need different graph models?~~ **No. The narrative graph is format-agnostic — same node/edge types for all formats.**

### Multi-File Projects (Resolved)
8. ~~Should we support Word (.docx) and PDF as source file inputs?~~ **Yes. Laires reads .docx (and .txt, .md, .fountain) directly — no conversion to a canonical format. Files are worked with in place.**
9. ~~Is EPUB export a priority?~~ **Deferred. Multi-file reading and editing is the priority.**
10. ~~Should export preserve the narrative graph as metadata?~~ **No. Export is clean output only — no graph metadata.**

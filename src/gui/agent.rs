...
use crate::concepts::manifest::{
    Manifest, build_classification_prompt, build_manifest_from_classification,
    diff_against_manifest, discover_files, parse_classification_response,
};
...
/// Classify discovered files via LLM and save the resulting manifest.
async fn classify_discovered_files(
    discovered: &[crate::concepts::manifest::DiscoveredFile],
    config: &crate::config::ProjectConfig,
    project_root: &std::path::Path,
    _events: &std::sync::mpsc::Sender<AgentEvent>,
) -> anyhow::Result<Manifest> {
    let prompt = build_classification_prompt(discovered);
    let class_model = if config.classification.model.is_empty() {
        &config.llm.model
    } else {
        &config.classification.model
    };
    let mut class_provider = Provider::from_project_config_with_model(config, class_model)
    ...
    let manifest = build_manifest_from_classification(result, discovered, class_model);
    manifest.save(project_root)?;
    Ok(manifest)
}

/// Run a story scan within the GUI context.
/// Re-discovers files on disk, re-classifies if new/removed files are found.
/// Falls back to the single loaded text_buffer/scene_map if no files are discovered.
async fn run_gui_scan(
    ...
    // Determine which manifest to use — always re-discover files to detect additions/removals
    ...
    let manifest = if let Some(ref existing) = d.manifest {
        ...
        if diff.new_files.is_empty() && diff.removed_files.is_empty() {
            ...
            let _ = events.send(AgentEvent::ToolCall {
                ...
                args_summary: format!(
                    "Found {} new, {} removed file(s). Classifying...",
                    diff.new_files.len(),
                    diff.removed_files.len()
                ),
            });
            classify_discovered_files(&discovered, &d.config, &project_root, events).await?
        }
    } else if Manifest::exists(&project_root) {
        let existing = Manifest::load(&project_root)?;
        let diff = diff_against_manifest(&discovered, &existing);
        if diff.new_files.is_empty() && diff.removed_files.is_empty() {
            d.manifest = Some(existing.clone());
            existing
        } else {
            let _ = events.send(AgentEvent::ToolCall {
                name: "scan_story".to_string(),
        ...
        // No manifest at all: classify everything
        let _ = events.send(AgentEvent::ToolCall {
            name: "scan_story".to_string(),
